...
// .wrangler/tmp/bundle-x4GaWd/checked-fetch.js
...
var init_live_feed_handler = __esm({
  "src/handlers/live-feed.handler.ts"() {
    ...
    init_response_utils();
    init_jwt_utils();
    getLiveFeed = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
      try {
    ...
    `;
        ...
        if (userId) {
          ...
          const followingStmt = env.DB.prepare(`
      ...
      `).bind(userId, ...creatorIds);
        ...
        return jsonResponse2(false, null, "FEED_ERROR", 500);
      }
    }, "getLiveFeed");
    getTrendingLive = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
      try {
        const url = new URL(request.url);
        const limit = parseInt(url.searchParams.get("limit") || "20");
        const query = `
      ...
      LIMIT ?
    `;
        const stmt = env.DB.prepare(query).bind(limit);
        const result = await stmt.all();
        return jsonResponse2(true, {
        ...
        console.error("Get trending live error:", error);
        return jsonResponse2(false, null, "TRENDING_ERROR", 500);
      }
    }, "getTrendingLive");
    getLiveByCategory = /* @__PURE__ */ __name(async (request, env, _ctx, params) => {
      try {
        const category = params?.category;
        if (!category) {
          return jsonResponse2(false, null, "CATEGORY_REQUIRED", 400);
        }
        const url = new URL(request.url);
        const limit = parseInt(url.searchParams.get("limit") || "20");
        const query = `
      ...
      LIMIT ?
    `;
        const stmt = env.DB.prepare(query).bind(category, limit);
        const result = await stmt.all();
        return jsonResponse2(true, {
          items: result.results || [],
          category,
        ...
        console.error("Get live by category error:", error);
        return jsonResponse2(false, null, "CATEGORY_ERROR", 500);
      }
    }, "getLiveByCategory");
    searchLiveStreams = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
      try {
        const url = new URL(request.url);
        const query = url.searchParams.get("q");
        const limit = parseInt(url.searchParams.get("limit") || "20");
        if (!query) {
          return jsonResponse2(false, null, "QUERY_REQUIRED", 400);
        }
        const searchQuery = `
      SELECT 
        ls.id,
    ...
    `;
        ...
        const stmt = env.DB.prepare(searchQuery).bind(searchTerm, searchTerm, searchTerm, limit);
        const result = await stmt.all();
        return jsonResponse2(true, {
          items: result.results || [],
          query,
        ...
        return jsonResponse2(false, null, "SEARCH_ERROR", 500);
      }
    }, "searchLiveStreams");
    batchLiveStats = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
      try {
        const body = await request.json();
        const { streamIds } = body;
        if (!streamIds || !Array.isArray(streamIds) || streamIds.length === 0) {
          return jsonResponse2(false, null, "STREAM_IDS_REQUIRED", 400);
        }
    ...
    `;
        const stmt = env.DB.prepare(query).bind(...streamIds);
        const result = await stmt.all();
        const stats = {};
        ...
        return jsonResponse2(true, { stats });
      } catch (error) {
        console.error("Batch live stats error:", error);
        return jsonResponse2(false, null, "BATCH_STATS_ERROR", 500);
      }
    }, "batchLiveStats");
  }
});

// .wrangler/tmp/bundle-x4GaWd/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-x4GaWd/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
// src/handlers/auth.handler.ts
init_jwt_utils();
var register = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  try {
...
var login = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  try {
    const body = await request.json().catch(() => ({}));
...
var refresh = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  try {
...
var forYouCore = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  try {
    const url = new URL(request.url);
...
var followingCore = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  try {
    const authHeader = request.headers.get("Authorization");
    if (!authHeader?.startsWith("Bearer ")) {
      return new Response(
...
// src/handlers/video.handler.ts
var uploadCore = /* @__PURE__ */ __name(async (request, env, ctx, _params) => {
  if (!env?.DB) {
    return new Response(
...
var getProcessingStatus = /* @__PURE__ */ __name(async (request, env, _ctx, params) => {
  const authResult = await requireAuth2(request, env);
  if (authResult instanceof Response) return authResult;
  const videoId = params.id;
  if (!env?.DB) {
...
var getConversations = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  const authResult = await requireAuth2(request, env);
  if (authResult instanceof Response) return authResult;
  const userId = authResult.userId;
  try {
...
var getAdsForFeed = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  const authResult = await requireAuth2(request, env);
  if (authResult instanceof Response) return authResult;
  const userId = authResult.userId;
  try {
...
var trackImpression = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  const authResult = await requireAuth2(request, env);
  if (authResult instanceof Response) return authResult;
  const userId = authResult.userId;
  try {
...
var trackClick = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  const authResult = await requireAuth2(request, env);
  if (authResult instanceof Response) return authResult;
  const userId = authResult.userId;
  try {
...
var createCampaign = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  const authResult = await requireAuth2(request, env);
  if (authResult instanceof Response) return authResult;
  const userId = authResult.userId;
  const ageCheck = await ensureMinimumAgeRegionAware(env, userId, { ip: request.headers.get("cf-connecting-ip") || void 0 });
...
var getCampaignAnalytics = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  const authResult = await requireAuth2(request, env);
  if (authResult instanceof Response) return authResult;
  const userId = authResult.userId;
...
var getRecommendedPricing = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  const authResult = await requireAuth2(request, env);
  if (authResult instanceof Response) return authResult;
  try {
    const url = new URL(request.url);
...
var createAdvertiserAccount = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  const authResult = await requireAuth2(request, env);
  if (authResult instanceof Response) return authResult;
  const userId = authResult.userId;
  try {
...
var getPricingTiers = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  try {
...
var uploadCreative = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  const authResult = await requireAuth2(request, env);
  if (authResult instanceof Response) return authResult;
  const userId = authResult.userId;
  try {
...
var getAdvertiserDashboard = /* @__PURE__ */ __name(async (request, env, _ctx, _params) => {
  const authResult = await requireAuth2(request, env);
  if (authResult instanceof Response) return authResult;
  const userId = authResult.userId;
  try {
...
async function recordConsent(request, env) {
  ...
  try {
    ...
    return jsonResponse2(false, { reason: error_.message || "ERROR" }, "CONSENT_WRITE_FAILED", 500);
  }
}
__name(recordConsent, "recordConsent");
async function getConsent(_request, env, _ctx, _params) {
...
async function withdrawConsent(request, env, _ctx, params) {
  ...
  try {
    ...
    return jsonResponse2(false, { reason: error_.message || "ERROR" }, "CONSENT_WITHDRAW_FAILED", 500);
  }
}
__name(withdrawConsent, "withdrawConsent");
async function eraseUserData(_request, env, _ctx, _params) {
  const userId = _params.userId;
  ...
  try {
    ...
    return jsonResponse2(false, null, "COMPLIANCE_ERASE_FAILED", 500);
  }
}
  ...
  try {
    ...
    if (/no such table/i.test(msg)) {
      try {
        ...
        return jsonResponse2(false, { reason: error_.message || "ERROR" }, "AGE_VERIFICATION_FAILED", 500);
      }
    }
...
async function auditUserCompliance(_req, env, _ctx, params) {
  ...
  try {
    ...
    return jsonResponse2(false, { reason: error_.message || "ERROR" }, "AUDIT_FAILED", 500);
  }
}
...
async function getDeviceMetricsAggregates(_req, env) {
  try {
...
var handleChatWebSocket = /* @__PURE__ */ __name(async (request, env, _ctx, params) => {
  try {
    ...
    const validateOrigin = /* @__PURE__ */ __name((originHeader) => {
      ...
      return ALLOWED_ORIGINS2.includes(originHeader) || env.NODE_ENV !== "production" && originHeader.startsWith("http://localhost");
    }, "validateOrigin");
...
init_checked_fetch();
init_modules_watch_stub();
var proxyBackend = /* @__PURE__ */ __name(async (request, env, _ctx, params) => {
  const backendOriginRaw = (env.BACKEND_ORIGIN || "http://localhost:5000").replace(/\/$/, "");
...
init_checked_fetch();
init_modules_watch_stub();
async function emitRuntimeTelemetry(env, payload, ctx) {
  const url = env.AI_RUNTIME_INGEST_URL;
  if (!url) return;
  const body = {
    ...payload,
    timestamp: payload.timestamp || (/* @__PURE__ */ new Date()).toISOString(),
    environment: env.AI_RUNTIME_ENVIRONMENT || env.ENVIRONMENT || "dev"
  };
  const headers = { "Content-Type": "application/json" };
  if (env.AI_RUNTIME_TOKEN) headers["Authorization"] = `Bearer ${env.AI_RUNTIME_TOKEN}`;
...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      await emitRuntimeTelemetry(env, {
